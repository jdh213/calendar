...
open class ActivityRingProgressBar : View {
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        ...
        parseAttributes(
            context.obtainStyledAttributes(
                attrs,
                R.styleable.ActivityRingProgressBar
            )
        )
    }
    ...
    private var mAnimationDurationCenterView = 2000
    ...
    private var mAnimationDurationInnerView = 2000
    ...
    private fun parseAttributes(a: TypedArray) {
        mOuterProgress =
            a.getInteger(R.styleable.ActivityRingProgressBar_outerProgress, mOuterProgress)
        mProgressColorOuterView = a.getColor(
            R.styleable.ActivityRingProgressBar_outerProgressColor,
            mProgressColorOuterView
        )
        mInnerProgress =
            a.getInteger(R.styleable.ActivityRingProgressBar_innerProgress, mInnerProgress)
        mProgressColorInnerView = a.getColor(
            R.styleable.ActivityRingProgressBar_innerProgressColor,
            mProgressColorInnerView
        )
        mCenterProgress =
            a.getInteger(R.styleable.ActivityRingProgressBar_centerProgress, mCenterProgress)
        ...
        hasTwoProgressView =
            a.getBoolean(R.styleable.ActivityRingProgressBar_hasTwoProgressView, hasTwoProgressView)
        mRoundedCorners =
            a.getBoolean(R.styleable.ActivityRingProgressBar_useRoundedCorner, mRoundedCorners)
        isAnimationOn =
            a.getBoolean(R.styleable.ActivityRingProgressBar_isAnimationOn, isAnimationOn)
        mStartAngleOuterView = a.getInteger(
            R.styleable.ActivityRingProgressBar_outerProgressStartAngle,
            mStartAngleOuterView
        )
        mStartAngleCenterView =
            a.getInteger(
                R.styleable.ActivityRingProgressBar_centerProgressStartAngle,
                mStartAngleCenterView
            )
        mStartAngleInnerView = a.getInteger(
            R.styleable.ActivityRingProgressBar_innerProgressStartAngle,
            mStartAngleInnerView
        )
        mMaxProgressOuterView = a.getInteger(
            R.styleable.ActivityRingProgressBar_outerMaxProgress,
            mMaxProgressOuterView
        )
        mMaxProgressInnerView = a.getInteger(
            R.styleable.ActivityRingProgressBar_innerMaxProgress,
            mMaxProgressInnerView
        )
        mMaxProgressCenterView = a.getInteger(
            R.styleable.ActivityRingProgressBar_centerMaxProgress,
            mMaxProgressCenterView
        )
        mElevation = a.getBoolean(R.styleable.ActivityRingProgressBar_hasElevation, mElevation)
        mEmptyProgressBar =
            a.getBoolean(R.styleable.ActivityRingProgressBar_hasEmptyProgressBar, mEmptyProgressBar)
        mEmptyProgressColorCenterView =
            a.getColor(
                R.styleable.ActivityRingProgressBar_centerEmptyProgressColor,
                mEmptyProgressColorCenterView
            )
        mEmptyProgressColorOuterView =
            a.getColor(
                R.styleable.ActivityRingProgressBar_outerEmptyProgressColor,
                mEmptyProgressColorOuterView
            )
        mEmptyProgressColorInnerView =
            a.getColor(
                R.styleable.ActivityRingProgressBar_innerEmptyProgressColor,
                mEmptyProgressColorInnerView
            )
        mCircleThickness =
            a.getFloat(R.styleable.ActivityRingProgressBar_circleThickness, mCircleThickness)
        mCirclePadding =
            a.getFloat(R.styleable.ActivityRingProgressBar_circlePadding, mCirclePadding)
        a.recycle()
    ...
    private fun drawProgressBackArc(
        ...
        backgroundPaint.strokeCap = if (mRoundedCorners) Paint.Cap.ROUND else Paint.Cap.ROUND
    ...
    fun setOuterProgress(progress: Int) {
        ...
        outerAnimator?.removeAllUpdateListeners()
    ...
    fun setInnerProgress(progress: Int) {
        if (!(hasOneProgressView && hasTwoProgressView)) {
            ...
            innerAnimator?.removeAllUpdateListeners()
            innerAnimator =
                ValueAnimator.ofFloat(
                    mSweepAngleInnerView.toFloat(),
            ...
            innerAnimator?.duration = if (progress <= 50) {
                mAnimationDurationInnerView.toLong()
            } else {
                mAnimationDurationInnerView.toLong() * 2
            }
            innerAnimator?.addUpdateListener { valueAnimator ->
                val value: Float = valueAnimator.animatedValue as Float
    ...
    fun setCenterProgress(progress: Int) {
        if (!(hasOneProgressView && !hasTwoProgressView)) {
            ...
            centerAnimator?.removeAllUpdateListeners()
            ...
            centerAnimator?.interpolator = DecelerateInterpolator()
            centerAnimator?.duration = if (progress <= 50) {
